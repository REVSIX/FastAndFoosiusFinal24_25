...
class ArduinoCOM:
    ...
    def send_positions(self, motorPos, servoPos):
        ...
        # #time.sleep(.001)  # Give Arduino time to respond
        # '''if self.ser.inWaiting() > 0:
        #     response = self.ser.readline().decode().rstrip()  # Read the response
        #     print(f"Received from Arduino: {response}")'''
        
        packet = motorPos + servoPos
        packet_str = ",".join(f"{x:.3f}" for x in packet) + "\n"
        self.ser.write(packet_str.encode())
        print(f"Sent to Arduino: {packet_str}")
