...
while True:  #infinite loop to keep processing data
    ...
    ret, frame = cap.read()  #ret is boolean for is feed is being captured, frame contains actual pixel data
    ...
    if len(contours) > 0: #if contours have been found
        ...
        if cv2.contourArea(c) > 50 and cv2.contourArea(c) > 500:  # Update this threshold as needed, 50 to 500
